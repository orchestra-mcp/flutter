          ...
          docsAsync.when(
            data: (docs) {
              final totalArticles = docs.fold<int>(
                0,
                (sum, d) => sum + ((d['article_count'] as num?)?.toInt() ?? 1),
              );
              return Text(
                AppLocalizations.of(context).nSectionsNArticles(docs.length, totalArticles),
...
class _DocTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final title = doc['title'] as String? ?? doc['name'] as String? ?? 'Untitled';
