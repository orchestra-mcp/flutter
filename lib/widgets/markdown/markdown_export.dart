...
String stripInlineMarkdown(String text) {
  ...
  result = result.replaceAllMapped(RegExp(r'\*{1,3}([^*]+)\*{1,3}'), (m) => m[1]!);
  result = result.replaceAllMapped(RegExp(r'_{1,3}([^_]+)_{1,3}'), (m) => m[1]!);
...
void _writeBlocksHtml(StringBuffer buf, List<MarkdownBlock> blocks) {
  for (final block in blocks) {
    switch (block) {
      case HeadingBlock():
        buf.writeln('<h${block.level}>${inlineToHtml(block.text)}</h${block.level}>');
      case ParagraphBlock():
        buf.writeln('<p>${inlineToHtml(block.text)}</p>');
      case CodeBlock():
        final lang = block.language.isNotEmpty ? ' class="language-${block.language}"' : '';
      ...
      case BlockquoteBlock():
        buf.writeln('<blockquote><p>${inlineToHtml(block.text)}</p></blockquote>');
      ...
      case TaskListBlock():
        ...
        for (final item in block.items) {
          ...
          buf.writeln('<li><input type="checkbox" $checked> ${inlineToHtml(item.text)}</li>');
        }
        buf.writeln('</ul>');
      case FrontmatterBlock():
        buf.writeln('<table><tbody>');
        for (final entry in block.data.entries) {
          buf.writeln('<tr><td><strong>${escapeHtml(entry.key)}</strong></td><td>${escapeHtml(entry.value)}</td></tr>');
        }
        buf.writeln('</tbody></table>');
...
String markdownToHtml(String content) {
  ...
  buf.writeln('<style>');
  buf.writeln('body { font-family: "IBM Plex Sans", "IBM Plex Sans Arabic", -apple-system, BlinkMacSystemFont, "Segoe UI", "Geeza Pro", "Al Nile", Roboto, sans-serif; max-width: 800px; margin: 40px auto; padding: 0 20px; line-height: 1.6; color: #1f2937; }');
  buf.writeln('h1, h2 { border-bottom: 1px solid #e5e7eb; padding-bottom: 8px; }');
  buf.writeln('code { font-family: "IBM Plex Mono", Consolas, monospace; background: #f3f4f6; padding: 2px 6px; border-radius: 4px; font-size: 0.9em; }');
  buf.writeln('pre { font-family: "IBM Plex Mono", Consolas, monospace; background: #1e1e2e; color: #d4d4d4; padding: 16px; border-radius: 8px; overflow-x: auto; }');
  buf.writeln('pre code { background: none; padding: 0; color: inherit; }');
  buf.writeln('blockquote { border-left: 3px solid #6366f1; margin: 0; padding: 8px 16px; background: rgba(99,102,241,0.05); }');
  buf.writeln('table { border-collapse: collapse; width: 100%; }');
  buf.writeln('th, td { border: 1px solid #e5e7eb; padding: 8px 12px; text-align: left; }');
...
String markdownToDocx(String content) {
  ...
  final buf = StringBuffer();
  buf.writeln('<!DOCTYPE html>');
  buf.writeln('<html dir="auto" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40">');
  buf.writeln('<head><meta charset="utf-8">');
  buf.writeln('<!--[if gte mso 9]><xml><w:WordDocument><w:View>Print</w:View></w:WordDocument></xml><![endif]-->');
  buf.writeln('<style>');
  buf.writeln('body { font-family: "IBM Plex Sans", "IBM Plex Sans Arabic", Calibri, "Geeza Pro", Arial, sans-serif; font-size: 11pt; line-height: 1.5; }');
  ...
  buf.writeln('code { font-family: "IBM Plex Mono", Consolas, monospace; background: #f3f4f6; padding: 1px 4px; }');
  buf.writeln('pre { font-family: "IBM Plex Mono", Consolas, monospace; background: #f3f4f6; padding: 12px; border: 1px solid #e5e7eb; }');
  buf.writeln('table { border-collapse: collapse; width: 100%; }');
  buf.writeln('th, td { border: 1px solid #d1d5db; padding: 6px 10px; }');
  buf.writeln('th { background: #f1f3f5; font-weight: bold; }');
  buf.writeln('blockquote { border-left: 3px solid #6366f1; margin: 0; padding: 8px 16px; }');
  buf.writeln('</style></head><body>');
...
class _PdfFonts {
  ...
  static Future<void> _ensureLoaded() async {
    _regular ??= pw.Font.ttf(await _loadAsset('assets/fonts/IBMPlexSans-Regular.ttf'));
    _bold ??= pw.Font.ttf(await _loadAsset('assets/fonts/IBMPlexSans-Bold.ttf'));
    _arabicRegular ??= pw.Font.ttf(await _loadAsset('assets/fonts/IBMPlexSansArabic-Regular.ttf'));
    _arabicBold ??= pw.Font.ttf(await _loadAsset('assets/fonts/IBMPlexSansArabic-Bold.ttf'));
    _mono ??= pw.Font.ttf(await _loadAsset('assets/fonts/IBMPlexMono-Regular.ttf'));
  }
...
Future<void> exportMarkdownAsPdf(String content, {String? title}) async {
  ...
  for (final block in blocks) {
    switch (block) {
      case HeadingBlock():
        ...
        final size = block.level <= sizes.length ? sizes[block.level - 1] : 11.0;
      ...
      case TableBlock():
        widgets.add(pw.Padding(
          ...
          child: pw.TableHelper.fromTextArray(
            ...
            headerDecoration: pw.BoxDecoration(color: PdfColor.fromHex('#F1F3F5')),
            cellPadding: const pw.EdgeInsets.all(6),
            border: pw.TableBorder.all(color: PdfColor.fromHex('#D1D5DB')),
            headers: block.headers.map(stripInlineMarkdown).toList(),
            data: block.rows.map((r) => r.map(stripInlineMarkdown).toList()).toList(),
          ),
        ));
      case BlockquoteBlock():
        widgets.add(pw.Padding(
        ...
        for (var i = 0; i < block.items.length; i++) {
          widgets.add(pw.Padding(
            ...
            child: pw.Row(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                pw.SizedBox(width: 20, child: pw.Text('${i + 1}.', style: baseStyle)),
                pw.Expanded(child: pw.Text(_pdfText(block.items[i]), style: baseStyle)),
              ],
            ),
          ));
        }
  ...
  final file = File('${dir.path}/${uniqueFileName('${title ?? 'document'}.pdf')}');
