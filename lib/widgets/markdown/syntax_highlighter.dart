...
Color syntaxColor(SyntaxTokenType type, {bool isDark = true}) {
  return switch (type) {
    SyntaxTokenType.keyword =>
      isDark ? const Color(0xFFC792EA) : const Color(0xFF7C3AED),
    SyntaxTokenType.string =>
      isDark ? const Color(0xFFA5D6A7) : const Color(0xFF16A34A),
    SyntaxTokenType.comment =>
      isDark ? const Color(0xFF6B7280) : const Color(0xFF9CA3AF),
    SyntaxTokenType.number =>
      isDark ? const Color(0xFFF78C6C) : const Color(0xFFEA580C),
    SyntaxTokenType.variable =>
      isDark ? const Color(0xFF82AAFF) : const Color(0xFF2563EB),
    SyntaxTokenType.type =>
      isDark ? const Color(0xFFFFCB6B) : const Color(0xFFCA8A04),
    SyntaxTokenType.punctuation =>
      isDark ? const Color(0xFF89DDFF) : const Color(0xFF6B7280),
...
List<_TokenRule> _buildRules(String lang) {
  ...
  return [
    ...
    // Dollar variables
    if (hasDollarVars)
      _TokenRule(RegExp(r'\$[a-zA-Z_]\w*'), SyntaxTokenType.variable),
...
List<SyntaxToken> tokenizeLine(String line, String language) {
  ...
  for (final m in resolved) {
    ...
    tokens.add(SyntaxToken(text: line.substring(m.start, m.end), type: m.type));
    pos = m.end;
  }
  if (pos < line.length) {
