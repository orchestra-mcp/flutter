...
final syncedNotesProvider = StreamProvider<List<Map<String, dynamic>>>((ref) {
  ...
  return db
      .watch('SELECT * FROM notes ORDER BY updated_at DESC')
      .map(
...
final syncedProjectsProvider = StreamProvider<List<Map<String, dynamic>>>((
  ref,
) {
  final db = ref.watch(powersyncDatabaseProvider);
  return db
...
final syncedWorkflowsProvider = StreamProvider<List<Map<String, dynamic>>>((
  ref,
) {
  final db = ref.watch(powersyncDatabaseProvider);
...
final syncedDelegationsProvider = StreamProvider<List<Map<String, dynamic>>>((
  ref,
) {
  final db = ref.watch(powersyncDatabaseProvider);
  return db
...
final syncedSessionsProvider = StreamProvider<List<Map<String, dynamic>>>((
  ref,
) {
  final db = ref.watch(powersyncDatabaseProvider);
  return db
      .watch('SELECT * FROM sessions ORDER BY updated_at DESC')
      .map(
        (results) => results.map((r) => Map<String, dynamic>.from(r)).toList(),
      );
});
...
final syncedUserSettingsProvider = StreamProvider<List<Map<String, dynamic>>>((
  ref,
) {
  final db = ref.watch(powersyncDatabaseProvider);
  return db
      .watch('SELECT * FROM user_settings ORDER BY updated_at DESC')
      .map(
        (results) => results.map((r) => Map<String, dynamic>.from(r)).toList(),
      );
});
...
final syncedHealthProfileProvider = StreamProvider<List<Map<String, dynamic>>>((
  ref,
) {
  final db = ref.watch(powersyncDatabaseProvider);
  return db
...
final syncedSleepConfigProvider = StreamProvider<List<Map<String, dynamic>>>((
  ref,
) {
  final db = ref.watch(powersyncDatabaseProvider);
  return db
...
final syncedHealthSnapshotsProvider =
