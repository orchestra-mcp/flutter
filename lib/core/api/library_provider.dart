...
final agentsProvider = StreamProvider<List<Map<String, dynamic>>>((ref) {
  ...
  return db.watch('SELECT * FROM agents ORDER BY updated_at DESC')
...
final skillsProvider = StreamProvider<List<Map<String, dynamic>>>((ref) {
  ...
  final db = ref.watch(powersyncDatabaseProvider);
  return db.watch('SELECT * FROM skills ORDER BY updated_at DESC')
      .map((r) => r.map((row) => Map<String, dynamic>.from(row)).toList());
});
...
final docsProvider = StreamProvider<List<Map<String, dynamic>>>((ref) {
  ...
  final db = ref.watch(powersyncDatabaseProvider);
  return db.watch('SELECT * FROM docs ORDER BY updated_at DESC')
      .map((r) => r.map((row) => Map<String, dynamic>.from(row)).toList());
});
...
final delegationsProvider = StreamProvider<List<Map<String, dynamic>>>((ref) {
  ...
  final db = ref.watch(powersyncDatabaseProvider);
  return db.watch('SELECT * FROM delegations ORDER BY updated_at DESC')
      .map((r) => r.map((row) => Map<String, dynamic>.from(row)).toList());
});

final plansProvider = StreamProvider.family<List<Map<String, dynamic>>, String?>((ref, projectSlug) {
  if (isDesktop) {
    final api = ref.watch(apiClientProvider);
  ...
  return db.watch('SELECT * FROM plans ORDER BY updated_at DESC')
      .map((r) => r.map((row) => Map<String, dynamic>.from(row)).toList());
});

final requestsProvider = StreamProvider.family<List<Map<String, dynamic>>, String?>((ref, projectSlug) {
  if (isDesktop) {
    final api = ref.watch(apiClientProvider);
  ...
  return db.watch('SELECT * FROM requests ORDER BY updated_at DESC')
      .map((r) => r.map((row) => Map<String, dynamic>.from(row)).toList());
});

final personsProvider = StreamProvider.family<List<Map<String, dynamic>>, String?>((ref, projectSlug) {
  if (isDesktop) {
    final api = ref.watch(apiClientProvider);
  ...
  return db.watch('SELECT * FROM persons ORDER BY updated_at DESC')
      .map((r) => r.map((row) => Map<String, dynamic>.from(row)).toList());
});
